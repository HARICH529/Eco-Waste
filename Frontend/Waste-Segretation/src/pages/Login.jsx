...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (result.success) {
            // Check role from result (assuming login returns user object or we can access it from context)
            // Ideally login function in context returns the user object or data
        ...
        <div className="min-h-[80vh] flex items-center justify-center pt-20 pb-12">
            <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-card border border-gray-100">
                ...
                <p className="mt-8 text-center text-gray-600 text-sm">
                    Don't have an account? <Link to="/register" className="text-primary-600 hover:text-primary-700 font-semibold hover:underline">Create account</Link>
                </p>
            </div>
        </div>
    );
